...
When referring to API docs, it has to do with ``docstrings``. For example in <ProgrammingLanguage />:
<RenderProgrammingLanguageBased target_name={"Rust"}>
```rust title="my_file.rs" lineNumbers
/// This is what docstrings are in Rust, right now we are documenting a function
fn abc(a: &str, b: i32, c: &[u8]) -> bool {
    // ...
}
```
...
<Callout type={"warning"} title={"Disclaimer About API Documentation Headers"}>
    ...
    see where and which headers are omitted, while also discussing it with the maintainer (McBrincie212).
</Callout>
...
- ``# Decorating / Wrapping Behavior`` Documents how ``TaskFrames`` behave when wrapping inner ``TaskFrame(s)``. **(REQUIRED)**
...
- ``# Checkpoint Behavior`` Describes how checkpoints are stored during TaskFrame execution. **(RECOMMENDED)**
...
**Type Alias Sections**
- ``# Semantics`` Elaborates the semantics of the type alias and its underlying type. **(REQUIRED)**
- ``# Generic(s)`` Describes the generics which the type alias may have (don't include it if there aren't any). **(RECOMMENDED)**
...
# When (Not) To Document an API
It is best to avoid document entirely **Private Items** such as internal struct/enum/func helpers, since they are never
use-facing, if you need to add clarification on something, best use comments.

Another reason to avoid documenting in heavy detail something is when it is a simple **Setter/Getter**, **Simple Type Alias** or
anything remotely close, for these cases. It is best to keep the description concise and short while explaining anything
that may be needed.